...
def pull_image(ctx, config):
    ...
    # The registry is occasionally unhealthy (quay.ceph.io has been seen
    # serving HTTP 504s and proxy-generated 404s for an image that is
    # present), which fails the pull on every host at once and kills the
    # job before bootstrap. Retry the hosts that failed.
